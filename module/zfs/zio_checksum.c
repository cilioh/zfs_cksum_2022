...
//#ifdef _KERNEL
//	printk(KERN_WARNING "[ZIO_CHECKSUM_COMPUTE]\n");
//#endif
	...
	if (ci->ci_flags & ZCHECKSUM_FLAG_EMBEDDED) {
		...
		//calling abd_checksum_SHA256 function
...
//		printk(KERN_WARNING "(%u)[ZIO--] %llu %llu %llu %llu\n", zio->id, (zcp)->zc_word[0], (zcp)->zc_word[1], (zcp)->zc_word[2], (zcp)->zc_word[3]);
//#endif
	}
...
//#ifdef _KERNEL
//	printk(KERN_WARNING "[CKSUM_ZIO_CHECKSUM_COMPUTE]\n");
//#endif
	...
	if (ci->ci_flags & ZCHECKSUM_FLAG_EMBEDDED) {
		...
		//ci->ci_func[0](abd, size, spa->spa_cksum_tmpls[checksum],
		//    &cksum);
		abd_checksum_SHA256(abd, size, NULL, &cksum);
		...
		//ci->ci_func[0](abd, size, spa->spa_cksum_tmpls[checksum],
		//    &bp->blk_cksum);
		abd_fletcher_4_native(abd, size, NULL, &bp->blk_cksum);
//#ifdef _KERNEL
//		zio_cksum_t *zcp = &bp->blk_cksum;
