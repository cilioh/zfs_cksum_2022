...
typedef enum zti_modes {
	...
	ZTI_MODE_BATCH_CKSUM,	/* cksum_modi */
...
//cksum_modi
#define ZTI_BATCH_CKSUM { ZTI_MODE_BATCH_CKSUM, 0, 1 }
...
const zio_taskq_info_t zio_taskqs[ZIO_TYPES][ZIO_TASKQ_TYPES] = {
	/* ISSUE	ISSUE_HIGH	INTR		INTR_HIGH	CKSUM */
	{ ZTI_ONE,	ZTI_NULL,	ZTI_ONE,	ZTI_NULL,	ZTI_ONE }, /* NULL */
	{ ZTI_N(8),	ZTI_NULL,	ZTI_P(12, 8),	ZTI_NULL,	ZTI_ONE }, /* READ */
	{ ZTI_BATCH,	ZTI_N(5),	ZTI_P(12, 8),	ZTI_N(5),	ZTI_BATCH_CKSUM }, /* WRITE */
...
//cksum_modi
uint_t		cksum_zio_taskq_batch_pct = 75;	/* 1 thread per cpu in pset */
...
spa_taskqs_init(spa_t *spa, zio_type_t t, zio_taskq_type_t q)
{
	...
	case ZTI_MODE_BATCH:
		...
		break;
	//cksum_modi
	case ZTI_MODE_BATCH_CKSUM:
		batch = B_TRUE;
		flags |= TASKQ_THREADS_CPU_PCT;
		value = MIN(cksum_zio_taskq_batch_pct, 100);
		break;
	...
	for (i = 0; i < count; i++) {
		...
		if (zio_taskq_sysdc && spa->spa_proc != &p0) {
			...
			//cksum_modi
			if (t == ZIO_TYPE_WRITE && q == ZIO_TASKQ_CKSUM)
...
//cksum_modi
module_param(cksum_zio_taskq_batch_pct, uint, 0444);
MODULE_PARM_DESC(cksum_zio_taskq_batch_pct,
	"Percentage of CPUs to run an cksum IO worker thread");
