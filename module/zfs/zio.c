...
int zio_opt = 0;
int zio_opt_iss = 0;
kmem_cache_t	*cksum_zio_cache;
int id = 0;

void cksum_zio_taskq_dispatch(zio_t *zio, zio_taskq_type_t q, boolean_t cutinline);
void _cksum_zio_taskq_dispatch(zio_t *zio);
static zio_t * zio_checksum_generate(zio_t *zio);

void create_cksum_zio(zio_t *zio);
void destroy_cksum_zio(zio_t *zio);
void barrier_cksum_zio(zio_t *zio);
...
//cksum_modi
	cksum_zio_cache =  kmem_cache_create("cksum_zio_cache",
	    sizeof (cksum_zio_t), 0, NULL, NULL, NULL, NULL, NULL, 0);
...
zio_fini(void)
{
	...
	//cksum_modi
	kmem_cache_destroy(cksum_zio_cache);
    ...
    enum zio_stage pipeline)
{
	...
	//taskq_init_ent(&zio->cksum_io_tqent);
//JW
//	if(zio_opt != 0)
//		create_cksum_zio(zio);
...
/*
	if(zio_opt != 0){
		// using cksum_zio_t structure - working
		//JW
		cksum_zio_t* cksum_zio = zio->cksum_zio;
		cksum_zio->cksum = 1;
		_cksum_zio_taskq_dispatch(zio);
		
		
		// using zio_t strucrure - not working - overlapped with compress func
		//zio->cksum = 1;
		//cksum_zio_taskq_dispatch(zio, ZIO_TASKQ_CKSUM, B_FALSE);
	}
	*/
...
//cksum_modi
static void
cksum_zio_checksum_generate(cksum_zio_t *cksum_zio)
...
//#ifdef _KERNEL
//	printk(KERN_WARNING "[%d]cksum_zio_checksum_generate\n", zio->id);
//#endif

	if (bp == NULL) {
		 //This is zio_write_phys().
		 //We're either generating a label checksum, or none at all.
		checksum = cksum_zio->io_prop.zp_checksum;
		...
		if (BP_IS_GANG(bp) && cksum_zio->io_child_type == ZIO_CHILD_GANG) {
	...
	//cksum_zio_checksum_compute(cksum_zio, zio, checksum, zio->io_abd, zio->io_size);
	
	cksum_zio_checksum_compute(cksum_zio, checksum, cksum_zio->io_abd, cksum_zio->io_size);
	
	cksum_zio->cksum = 2;
	
	//destroy cksum_zio_t structure
	//if(i)
	//taskq_destroy(cksum_zio->stqs_taskq[0]);
	//kmem_free(cksum_zio->stqs_taskq, sizeof(taskq_t *));
	/*
	abd_free(cksum_zio->io_abd);
	kmem_free(cksum_zio->io_bp, sizeof (blkptr_t));
	kmem_cache_free(cksum_zio_cache, cksum_zio);
	*/	
	return (NULL);
}
...
/*	
	if(t == ZIO_TYPE_NULL){
		spa_taskq_dispatch_ent(spa, t, ZIO_TASKQ_ISSUE, (task_func_t *)zio_execute, zio, flags, &zio->io_tqent);
		return;
	}
*/
	...
	if (zio_opt != 0){
		//ORIGINAL+DYNAMIC
		//ctxt = get_iss_ctxt(tq);
		ctxt = get_cks_ctxt(tq);

		//if(ctxt != ctxt_prev_z){
			//calculate current context switch overhead
		diff = ctxt - ctxt_prev_z;
			//diff - diff_prev_z : difference of context switch overhead
			//if(diff != diff_prev_z){
		tq->tq_ctxt_d = diff - diff_prev_z;
			//}
		diff_prev_z = diff;
		tq->tq_ctxt = diff;
		//}
		...
		if(zio_opt == 1){
			if (tq->tq_ctxt_d > 0) {
				taskq_dispatch_ent(tq, (task_func_t *)zio_checksum_generate, zio, flags, &zio->io_tqent);
			}
			else {
				zio_checksum_generate(zio);
			}
		}
		else{
...
int diff_prev_y = 0;
void
iss_zio_taskq_dispatch(zio_t *zio, zio_type_t t, zio_taskq_type_t q, boolean_t cutinline)
{
	spa_t *spa = zio->io_spa;
	//zio_type_t t = zio->io_type;
	int flags = (cutinline ? TQ_FRONT : 0);
...
//	ASSERT3U(q, <, ZIO_TASKQ_TYPES);
	//if (t == ZIO_TYPE_WRITE){
		...
		//calculate current context switch overhead
		diff = ctxt - ctxt_prev_y;
		//diff - diff_prev_y : difference of context switch overhead
		tq->tq_ctxt_d = diff - diff_prev_y;
		diff_prev_y = diff;
		tq->tq_ctxt = diff;
		...
		//whether to dispatch more thread or not
		if (tq->tq_ctxt_d > 0) {
			taskq_dispatch_ent(tq, (task_func_t *)zio_execute, zio, flags, &zio->io_tqent);
		}
		else {
			zio_execute(zio);
		}
...
/*		
		...
		spa_taskq_dispatch_ent(spa, t, q, (task_func_t *)zio_execute, zio, flags, &zio->io_tqent);
	}
	*/
}
...
d_local[0] = gethrtime();
	...
	cksum_zio = kmem_cache_alloc(cksum_zio_cache, KM_SLEEP);
	bzero(cksum_zio, sizeof(cksum_zio_t));

	//cksum_modi
	cksum_zio->io_size = zio->io_orig_size;
	cksum_zio->io_offset = zio->io_offset;

	if(zio->io_orig_abd != NULL){
		//cksum_modi
		cksum_zio->io_abd = zio->io_orig_abd;
		//cksum_zio->io_abd = abd_alloc_sametype(zio->io_abd, (zio->io_abd)->abd_size);
		//abd_copy_cksum(cksum_zio->io_abd, zio->io_abd, zio->io_size);
		
		//memcpy(cksum_zio->io_abd, zio->io_abd, (zio->io_abd)->abd_size);
//#ifdef _KERNEL
//		printk(KERN_WARNING "size:%llu\n", zio->io_size);
//#endif
	}

	blkptr_t *cksum_bp = kmem_alloc(sizeof (blkptr_t), KM_SLEEP);
	BP_ZERO(cksum_bp);
	cksum_zio->io_bp = cksum_bp;

	cksum_zio->io_type = zio->io_type;
	cksum_zio->io_prop = zio->io_prop;
	cksum_zio->io_child_type = zio->io_child_type;

	cksum_zio->cksum = 0;

	cksum_zio->id = zio->id;

	zio->cksum_zio = cksum_zio;	
...
/*
	cksum_zio_t* cksum_zio = zio->cksum_zio;
	if(cksum_zio->cksum == 1){
		while(cksum_zio->cksum != 2){
			barrier();
		}
	}
	
	if(cksum_zio->cksum == 2){
		blkptr_t *bp = zio->io_bp;
		blkptr_t *cksum_bp = cksum_zio->io_bp;
		zio_cksum_t *zcp = &bp->blk_cksum;
		zio_cksum_t *cksum_zcp = &cksum_bp->blk_cksum;
		
		//printk(KERN_WARNING "[ZIO] %llu %llu %llu %llu\n", (zcp)->zc_word[0], (zcp)->zc_word[1], (zcp)->zc_word[2], (zcp)->zc_word[3]);
		//printk(KERN_WARNING "[CKSUM_ZIO] %llu %llu %llu %llu\n", (cksum_zcp)->zc_word[0], (cksum_zcp)->zc_word[1], (cksum_zcp)->zc_word[2], (cksum_zcp)->zc_word[3]);
	
		(zcp)->zc_word[0] = (cksum_zcp)->zc_word[0];
		(zcp)->zc_word[1] = (cksum_zcp)->zc_word[1];
		(zcp)->zc_word[2] = (cksum_zcp)->zc_word[2];
		(zcp)->zc_word[3] = (cksum_zcp)->zc_word[3];
	}
	*/
#endif
	h_local[1] = gethrtime();
...
int zio_ctxt = 0;
void
_cksum_zio_taskq_dispatch(zio_t *zio)
{
	cksum_zio_t* cksum_zio = zio->cksum_zio;

	//ctxt_modi
	spa_t *spa = zio->io_spa;
	spa_taskqs_t *tqs = &spa->spa_zio_taskq[zio->io_type][ZIO_TASKQ_CKSUM];
	taskq_t *tq = tqs->stqs_taskq[0];
	int flags = (B_FALSE ? TQ_FRONT : 0);
	int ctxt = 0;
	int diff = 0;
	...
	if (zio_opt != 0){
		...
		if(ctxt != ctxt_prev){
			//calculate current context switch overhead
			diff = ctxt - ctxt_prev;
			//diff - diff_prev : difference of context switch overhead
			if(diff != diff_prev){
				tq->tq_ctxt_d = diff - diff_prev;
			}
			diff_prev = diff;
			tq->tq_ctxt = diff;
		}
...
zio_taskq_dispatch(zio_t *zio, zio_taskq_type_t q, boolean_t cutinline)
{
	...
	/*spa_taskqs_t *tqs;
	taskq_t *tq;
	tqs = &spa->spa_zio_taskq[t][ZIO_TASKQ_ISSUE];
	tq = tqs->stqs_taskq[0];
	if (zio_opt == 1 && q == ZIO_TASKQ_ISSUE && tq->tq_nactive >= zio_nactive){
#ifdef _KERNEL
		printk(KERN_WARNING "tq->tq_nactive:%d\n", tq->tq_nactive);
#endif
	}*/
	...
	//JW0616
	//if(zio_opt_iss == 1 && t == ZIO_TYPE_WRITE && q == ZIO_TASKQ_ISSUE)
	//	iss_zio_taskq_dispatch(zio, t, q, cutinline);
	//else
...
//#ifdef _KERNEL
//	printk(KERN_WARNING "[%d] zio_issue_async\n", zio->id);
//#endif
	
	//ORIGINAL+DYNAMIC
	//iss_zio_taskq_dispatch(zio, ZIO_TASKQ_ISSUE, B_FALSE);
...
__zio_execute(zio_t *zio)
{
	...
	while (zio->io_stage < ZIO_STAGE_DONE) {
		...
		if(zio_opt != 0 && highbit64(stage)-1 == 4){
			stage <<= 1;
		}
		else if(zio_opt != 0 && highbit64(stage)-1 == 6){
		//	barrier_cksum_zio(zio);
			zio->cksum = 1;
			cksum_zio_taskq_dispatch(zio, ZIO_TASKQ_CKSUM, B_FALSE);
			stage <<= 1;
		}
		...
		if(zio_opt == 0 && highbit64(stage)-1 == 4){
#ifdef _KERNEL
			spa_t *spa = zio->io_spa;
			spa_taskqs_t *tqs = &spa->spa_zio_taskq[zio->io_type][ZIO_TASKQ_ISSUE];
			taskq_t *tq;
			if(tqs->stqs_count == 1)
				tq = tqs->stqs_taskq[0];
			else
			...
			ctxt = get_iss_ctxt(tq);
			if(ctxt != ctxt_prev__){
				diff = ctxt-ctxt_prev__;
				if(diff != diff_prev__)
					tq->tq_ctxt_d = diff - diff_prev__;
				diff_prev__ = diff;
				tq->tq_ctxt = diff;
			}
			ctxt_prev__ = ctxt;
...
//cksum_modi: BARRIER_REMOVE
	if(zio_opt != 0)
		barrier_cksum_zio(zio);

	//calling zio_dva_throttle
...
//cksum_modi
	zio_checksum_compute(zio, checksum, zio->io_abd, zio->io_size);
	//zio_checksum_compute(zio, checksum, zio->io_orig_abd, zio->io_orig_size);
...
//cksum_modi
	...
	//zio_cksum_t *zcp = &bp->blk_cksum;
	error = cksum_zio_checksum_error(zio, &info);
	//52 = EBADE invalid exchange

#ifdef _KERNEL
	if(error != 0)
		printk(KERN_WARNING "ERROR : %d\n", error);
	//printk(KERN_WARNING "%llu %llu %llu %llu\n", (zcp)->zc_word[0], (zcp)->zc_word[1], (zcp)->zc_word[2], (zcp)->zc_word[3]);
#endif
...
MODULE_PARM_DESC(zio_opt, "zio_optimization on/off");

module_param(zio_opt_iss, int, 0644);
MODULE_PARM_DESC(zio_opt_iss, "zio_optimization on/off");
